...
function goToUrl(url, target) {
    if (url.indexOf("mailto:") != -1) {
        ...
        if (target == "_self") {
            ...
            wins[wNum] = open(url, "", "status=yes,scrollbars=yes,scrolling=yes,toolbar=yes,menubar=yes,location=yes,resizable=yes");
            wNum++;
        } else {
            if (top[target]) {
                top[target].document.location = url;
            } else {
                document.location = url;
            }
        }
...
function openSubMenu(mid) {
	...
	for(i=0;i<sI[mid].length;i++){
		...
		// El ultimo lo fijo yo
		if(i != sI[mid].length - 1){
			shAry[i]=subNavH;
		} else {
			//alert(sI[mid].length - 1);
			shAry[sI[mid].length - 1] = 6;
		}
		...
		textScale='width:auto;min-width:'+(sP[mid][0]-(2*sbSIZE))+'px\;min-';
